# from fileutils tmpdir find tempfile

# combine File Dir FileUtils

class Pa < File 
	Error = Class.new Exception

	attr_reader :path, :name, :fname, :ext, :fext, :absolute, :relative
	def initialize path, relative=nil
		@path = path
		@name = Pa.basename(@path, true)[0] 
		@fname = Pa.basename(@path) 
		@ext = Pa.extname(@path) 
